...
router.post('/', async (req: AuthRequest, res) => {
  try {
    const { lectureId, timestamp, timestampLabel, content, isImportant } = req.body;
    if (!lectureId || !content) {
      res.status(400).json({ message: 'Lecture ID and content are required' });
      return;
    }
    const note = await Note.create({
      ...
      timestampLabel: timestampLabel || '0:00',
      content,
